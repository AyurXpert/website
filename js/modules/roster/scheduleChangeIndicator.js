...
export async function checkRosterChanged(supabase, tenantId, profileId) {
  const { data: prof } = await supabase.from('profiles').select('roster_last_seen_at').eq('id', profileId).maybeSingle();
  ...
  const { count } = await supabase.from('duty_roster')
...
export async function markRosterSeen(supabase, profileId) {
  await supabase.from('profiles').update({ roster_last_seen_at: new Date().toISOString() }).eq('id', profileId);
}
